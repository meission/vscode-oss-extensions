...
exports.addSDLProviders = addSDLProviders;
...
function completeDubVersion(info) {
    ...
    return new Promise((resolve) => {
        dub_api_1.getPackageInfo(packageName).then(json => {
...
function platformPatternCompleter(info) {
...
class SDLContributions {
    provideCompletionItems(document, position, token) {
        console.log("Completion:");
        try {
            var info = sdlinfo_1.getLocationInfo(document, position);
            console.log(info);
            if (info.namespace[0] != "")
                return Promise.reject("Invalid namespace in info object");
            if (info.name[0] != "")
                return Promise.reject("Invalid name in info object");
            if (info.namespace.length != info.name.length)
                return Promise.reject("Invalid info object (length mismatch)");
            if (info.namespace.length <= 0)
                return Promise.reject("Invalid info object (no entry)");
            let obj = dubSchema;
                ...
                if (obj && obj.values) {
                    if (obj.values.pattern) {
                        if (obj.values.pattern.complete) {
                            return Promise.resolve(obj.values.pattern.complete(info)).then((values) => {
    ...
    provideDiagnostics(document) {
        ...
        function scanTag(tag, obj, nsName = "") {
            if (obj.tags) {
                var hasTags = false;
...
exports.SDLContributions = SDLContributions;
//# sourceMappingURL=sdl-contributions.js.map