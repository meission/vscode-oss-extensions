...
class CompileButtons {
    constructor(served) {
        this.isDebug = false;
        this.debugValuesCache = null;
        this.served = served;
    ...
    run() {
        this.isDebug = false;
        vscode.tasks.fetchTasks({ type: "dub" }).then(tasks => {
            var used;
            tasks.forEach(task => {
                if (!task.definition.test && task.definition.run)
                    used = task;
            });
            if (used)
                vscode.tasks.executeTask(used);
        });
    }
    build() {
        this.isDebug = false;
    ...
    startProc(cmd, inTerminal = false) {
        if (inTerminal) {
            ...
            Promise.all([
                this.served.client.sendRequest("served/getConfig"),
                this.served.client.sendRequest("served/getArchType"),
                this.served.client.sendRequest("served/getBuildType"),
                this.served.client.sendRequest("served/getCompiler")
            ]).then(values => {
                if (this.terminal) {
            ...
            Promise.all([
                ...
                this.child = ChildProcess.spawn(extension_1.config(null).get("dubPath", "dub"), args, { cwd: vscode.workspace.rootPath, detached: true });
    ...
    handleStop(code) {
        ...
        if (this.isDebug && code == 0) {
            this.isDebug = false;
            var proc = ChildProcess.spawn(extension_1.config(null).get("dubPath", "dub"), [
                "describe",
...
exports.CompileButtons = CompileButtons;
//# sourceMappingURL=compile-buttons.js.map