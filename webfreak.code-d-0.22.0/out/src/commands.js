...
function registerClientCommands(context, client, served) {
    ...
    subscriptions.push(vscode.tasks.registerTaskProvider("dub", new dub_tasks_1.DubTasksProvider(client)));
    gClient = client;
    ...
    subscriptions.push(vscode.commands.registerCommand("code-d.addDependency", () => {
        vscode.window.showQuickPick(dub_api_1.listPackageOptions(), {
    ...
    subscriptions.push(vscode.commands.registerCommand("code-d.updateDependency", (node) => {
        if (node.info)
            dub_api_1.getLatestPackageInfo(node.info.name).then((info) => {
                if (node.info)
...
function registerCommands(context) {
    ...
    let rdmdTerminal;
    subscriptions.push(vscode.commands.registerCommand("code-d.rdmdCurrent", (file) => {
        var args = [];
    ...
    subscriptions.push(vscode.commands.registerCommand("code-d.createProject", () => {
        project_creator_1.showProjectCreator(context);
    }));
            ...
            for (var i = 0; i < templates.length; i++)
                if (templates[i].path == id) {
                    ...
                    fs.readdir(path, function (err, files) {
                        if (files.length == 0)
                            project_creator_1.performTemplateCopy(context, id, templates[i].dub, path, function () {
                            ...
                            vscode.window.showWarningMessage("The current workspace is not empty!", "Select other Folder", "Merge into Folder").then(r => {
                                if (r == "Select other Folder") {
                                    ...
                                    project_creator_1.openFolderWithExtension(context);
                                }
    ...
    subscriptions.push(vscode.commands.registerCommand("code-d.searchDocs", () => {
        ...
        dpldocs_1.showDpldocsSearch(query);
    }));
    subscriptions.push(vscode.commands.registerTextEditorCommand("code-d.openDocsAtCursor", (editor, edit) => {
        ...
        if (editor.selection.isEmpty) {
            ...
            if (range)
                dpldocs_1.showDpldocsSearch(editor.document.getText(range), true);
            else
                dpldocs_1.showDpldocsSearch("");
        }
        else
            dpldocs_1.showDpldocsSearch(editor.document.getText(editor.selection), true);
    }));
