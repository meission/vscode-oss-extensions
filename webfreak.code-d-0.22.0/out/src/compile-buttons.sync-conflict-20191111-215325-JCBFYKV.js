...
class CompileButtons {
    ...
    startProc(cmd, inTerminal = false) {
        if (inTerminal) {
            ...
            Promise.all([
                ...
                this.child = ChildProcess.spawn(extension_1.config().get("dubPath", "dub"), args, { cwd: vscode.workspace.rootPath, detached: true });
    ...
    handleStop(code) {
        ...
        if (this.isDebug && code == 0) {
            this.isDebug = false;
            var proc = ChildProcess.spawn(extension_1.config().get("dubPath", "dub"), [
                "describe",
