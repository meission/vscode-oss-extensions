    ...
    return new Promise((resolve, reject) => {
        ...
        request.get({ url: filePath, encoding: "binary" }, (error, response, body) => {
            if (error) {
                ...
                const localFilePath = path.resolve(DOWNLOADS_TEMP_FOLDER, compute_checksum_1.default(filePath)) +
 ...
 * @param param3
 */
function transformMarkdown(inputString, { fileDirectoryPath = "", projectDirectoryPath = "", filesCache = {}, useRelativeFilePath = null, forPreview = false, forMarkdownExport = false, protocolsWhiteListRegExp = null, notSourceFile = false, imageDirectoryPath = "", usePandocParser = false, headingIdGenerator = new heading_id_generator_1.default(), onWillTransformMarkdown = null, onDidTransformMarkdown = null, }) {
    return __awaiter(this, void 0, void 0, function* () {
                ...
                while (i < inputString.length) {
                    ...
                    if (line.match(/^(\!\[|@import)/) &&
                        ...
                        if (optMatch) {
                            ...
                            try {
                                opt = attributes_1.parseAttributes(optMatch[0]);
                                (classes = opt["class"]),
                                    (id = opt["id"]),
                                    (ignore = opt["ignore"]);
                                delete opt["class"];
                    ...
                    if (refMatch) {
                        ...
                        const fileContent = engine_server_1.getProcessor({ root, renderWithOutline: false })
                            .processSync(line)
                            .toString();
                        let output2 = fileContent;
                        output2 = "\n" + output2 + "  ";
                        i = end + 1;
                        lineNo = lineNo + 1;
                    ...
                    if (importMatch) {
                        ...
                        if ([".jpeg", ".jpg", ".gif", ".png", ".apng", ".svg", ".bmp"].indexOf(extname) >= 0) {
                            ...
                            const output2 = `\`\`\`text ${attributes_1.stringifyAttributes(config)}  \n\`\`\`  `;
                            // return helper(end+1, lineNo+1, outputString+output+'\n')
                            i = end + 1;
                            lineNo = lineNo + 1;
                            outputString = outputString + output2 + "\n";
                            ...
                            try {
                                ...
                                if (config && config["code_block"]) {
                                    ...
                                    output = `\`\`\`${config["as"] ||
                                        ...
                                        fileExtension} ${attributes_1.stringifyAttributes(config)}  \n${fileContent}\n\`\`\`  `;
                                }
                                    ...
                                    */
                                    ...
                                    if (config && config["code_block"] === false) {
                                        ...
                                        output = `\`\`\`${aS ||
                                            fileExtensionToLanguageMap[fileExtension] ||
                                            fileExtension} ${config ? attributes_1.stringifyAttributes(config) : ""}  \n${fileContent}\n\`\`\`  `;
                                    }
                                }
                                // return helper(end+1, lineNo+1, outputString+output+'\n')
                                i = end + 1;
