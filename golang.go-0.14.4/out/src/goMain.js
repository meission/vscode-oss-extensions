...
function activate(ctx) {
    ...
    stateUtils_1.setWorkspaceState(ctx.workspaceState);
    goInstallTools_1.updateGoPathGoRootFromConfig().then(() => __awaiter(this, void 0, void 0, function* () {
        ...
        const ok = yield goLanguageServer_1.registerLanguageFeatures(ctx);
        if (!ok) {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('go.locate.tools', () => __awaiter(this, void 0, void 0, function* () {
        goStatus_1.outputChannel.show();
        goStatus_1.outputChannel.clear();
        goStatus_1.outputChannel.appendLine('Checking configured tools....');
        ...
        // 5) GOROOT
        // 6) PATH
        goStatus_1.outputChannel.appendLine('GOBIN: ' + process.env['GOBIN']);
        goStatus_1.outputChannel.appendLine('toolsGopath: ' + util_1.getToolsGopath());
        goStatus_1.outputChannel.appendLine('gopath: ' + util_1.getCurrentGoPath());
        goStatus_1.outputChannel.appendLine('GOROOT: ' + process.env['GOROOT']);
        goStatus_1.outputChannel.appendLine('PATH: ' + process.env['PATH']);
        ...
        allTools.forEach((tool) => {
            ...
            goStatus_1.outputChannel.appendLine(`   ${tool.name}: ${toolPath} ${msg}`);
        });
    })));
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('go.apply.coverprofile', () => {
        ...
        const lastCoverProfilePath = stateUtils_1.getFromWorkspaceState(lastCoverProfilePathKey, '');
        vscode.window
            .showInputBox({
            ...
            if (coverProfilePath !== lastCoverProfilePath) {
                stateUtils_1.updateWorkspaceState(lastCoverProfilePathKey, coverProfilePath);
            }
...
function deactivate() {
    return Promise.all([testUtils_1.cancelRunningTests(), Promise.resolve(util_1.cleanupTempDir())]);
}
...
function addOnSaveTextDocumentListeners(ctx) {
    vscode.workspace.onDidSaveTextDocument(goCover_1.removeCodeCoverageOnFileSave, null, ctx.subscriptions);
...
function addOnChangeTextDocumentListeners(ctx) {
    vscode.workspace.onDidChangeTextDocument(goCover_1.trackCodeCoverageRemovalOnFileChange, null, ctx.subscriptions);
