...
class CommittedFileItem extends vscode_1.TreeItem {
...
function buildOneFileWithFolder(rootFolder, file, relativePath = '') {
    const segments = relativePath ? path.relative(relativePath, file.gitRelativePath).split(/\\|\//) :
...
class ExplorerViewProvider {
    constructor(model, _gitService) {
        ...
        this._disposables.push(vscode_1.commands.registerCommand('githd.viewFileHistoryFromTree', (fileItem) => model.setHistoryViewContext({ repo: this._context.repo, specifiedPath: fileItem.file.fileUri })));
