    ...
    _updateContent() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!loadingMore) {
                ...
                utils_1.decorateWithoutWhitespace(this._titleDecorationOptions, this._content, 0, 0);
                if (!isStash) {
            ...
            entries.forEach(entry => {
                ++this._logCount;
                utils_1.decorateWithoutWhitespace(this._subjectDecorationOptions, entry.subject, this._currentLine, 0);
                ...
                if (entry.ref) {
                    ...
                    info += entry.ref;
                    utils_1.decorateWithoutWhitespace(this._refDecorationOptions, entry.ref, this._currentLine, start);
                }
                if (entry.author) {
                    info += ' by ';
                    let start = info.length;
                    info += entry.author;
                    utils_1.decorateWithoutWhitespace(this._authorDecorationOptions, entry.author, this._currentLine, start);
                }
                if (entry.email) {
                    info += ' <';
                    let start = info.length;
                ...
                if (entry.date) {
                    ...
                    let start = info.length;
                    info += entry.date;
                    utils_1.decorateWithoutWhitespace(this._dateDecorationOptions, entry.date, this._currentLine, start);
                }
                this._content += info + '\n';
                ++this._currentLine;
                if (entry.stat) {
