...
class Builder {
    ...
    buildInitiator(rootFile, languageId, recipe = undefined, releaseBuildMutex) {
    ...
    async build(rootFile, languageId, recipe = undefined) {
        if (this.isWaitingForBuildToFinish()) {
    ...
    createSteps(rootFile, languageId, recipeName) {
        ...
        if (recipeName === undefined && magicTex && !configuration.get('latex.build.forceRecipeUsage')) {
            ...
            const defaultRecipe = configuration.get('latex.recipe.default');
